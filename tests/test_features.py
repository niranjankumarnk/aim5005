...
class TestFeatures(TestCase):
    ...
    def test_min_max_fit(self):
        ...
        scaler.fit(data)
        assert np.allclose(scaler.maximum, np.array([1., 18.])), "scaler fit does not return maximum values [1., 18.] "
        assert np.allclose(scaler.minimum, np.array([-1., 2.])), "scaler fit does not return maximum values [-1., 2.] " 
    ...
    def test_min_max_scaler_single_value(self):
        ...
        assert np.allclose(result, expected), "Scaler transform does not return expected values. Expect [[1.5 0. ]]. Got: {}".format(result)
    ...
    def test_standard_scaler_transform(self):
        ...
        scaler.fit(data)
        result = scaler.transform(data)
        assert np.all(result, expected), "Scaler transform does not return expected values. Expect {}. Got: {}".format(expected.reshape(1,-1), result.reshape(1,-1))
    ...
    def standard_scaler_standardization_test(self):
        data = [[1,2,3],[4,5,6],[7,8,9]]
        expected_mean = np.zeros(len(data[0]))
        expected_std = np.ones(len(data[0]))
        scaler = StandardScaler()
        scaler.fit(data)
        result = scaler.transforn(data)
        assert np.allclose(np.mean(result, axis = 0), expected_mean),\
            " Scaler transform does not return data with expected mean {}. Got {}".format(expected_mean,np.mean(result.axis = 0))
        assert np.allclose(np.std(result, axis = 0), expected_std), \
            "Scaler transform does not return data with expected standard deviation {}. Got {}".format(expected_std, np.std(result, axis=0))
